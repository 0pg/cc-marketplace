...
각 요구사항의 RED 단계 진입 전, 테스트 작성 난이도를 평가합니다.

**테스트가 어려운 경우 = 설계 피드백 신호**

테스트 작성이 어렵다면 건너뛰지 말고, 원인을 진단하고 프로덕션 코드를 리팩토링합니다.

| 증상 | 원인 | 처방 |
|------|------|------|
| Mock 불가능한 정적/전역 호출 | 하드코딩된 의존성 | 인터페이스 추출 + 의존성 주입 |
| 테스트에 과도한 setup 필요 | 숨겨진 의존성 다수 | 책임 분리, 생성자 주입으로 의존성 명시화 |
| 시간/난수 등 비결정적 동작 | 전역 상태 직접 참조 | 추상화 계층 도입 (Clock, RNG trait) |
| 하나의 동작 검증에 여러 Mock 필요 | 과도한 결합 | 파사드 추출 또는 책임 재분배 |

**프로토콜:**
1. 테스트 코드를 머릿속으로 스케치
2. 어렵다면: 원인을 위 표에서 식별
3. 프로덕션 코드를 먼저 리팩토링 (기존 테스트 green 유지)
4. 리팩토링 완료 후 RED 진입
