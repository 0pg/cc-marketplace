...
description: |
  ...
  CLAUDE.md Exports가 있으면 구조적 불변식(STRUCT-XXX)을 추출합니다.
  "테스트 설계", "테스트 케이스 도출", "tdd-spec 작성", "Exports에서 불변식 추출", "구조적 불변식 테스트", "STRUCT 테스트 설계" 요청 시 사용됩니다.
---
...
## Phase 1.5: CLAUDE.md Exports 기반 구조적 불변식
...
**프로세스:**
1. 대상 디렉토리에 CLAUDE.md가 존재하는지 확인
2. Exports 파싱: functions, types, classes, enums, variables
3. 각 Export에 대해 STRUCT-XXX 요구사항 생성
4. Contract 섹션이 있으면 사전/사후조건을 STRUCT에 포함
5. tdd-spec.md에 STRUCT 섹션 추가
...
- CLAUDE.md Exports 기반 STRUCT-XXX 형식으로 구조적 불변식 구조화 (Phase 1.5 결과)
