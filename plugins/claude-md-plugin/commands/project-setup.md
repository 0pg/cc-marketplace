...
| 코딩 규칙 | 비동기 처리 패턴, 에러 처리 패턴, 타입 사용 규칙 등 |
| 네이밍 규칙 | 변수/함수/클래스/상수의 네이밍 패턴 분석 |
| import 패턴 | import 순서, 그루핑 규칙 |
| 포맷터/린터 설정 | `.prettierrc`, `.eslintrc`, `ruff.toml` 등 — **제외 판단용** (린터가 처리하는 항목을 Convention에서 제외) |
...
### Coding Rules
(필수) 린터가 검증 불가한 기본 코딩 규칙 (비동기 패턴, 타입 사용 규칙, 불변성 등)
