...
| 코드 작성 패턴 | 함수 설계 패턴(early return, guard clause), 불변성 규칙, 비동기 패턴, null 처리 전략, 조건문 스타일, 에러 처리 패턴 등 |
| 네이밍 규칙 | 변수/함수/클래스/상수의 네이밍 패턴 분석 |
| import 패턴 | import 순서, 그루핑 규칙 |

> **참고**: 들여쓰기, 따옴표, 세미콜론, 줄 길이 등 포맷팅 규칙은 lint/formatter(`.prettierrc`, `.eslintrc`, `ruff.toml` 등)가 자동으로 처리하므로 추출 대상에서 제외합니다. lint로 강제할 수 없는 코드 작성 패턴만 추출합니다.
...
### Code Style
(필수) 코드 작성 패턴, 설계 규칙 (lint로 강제할 수 없는 규칙)
