...
상세 알고리즘은 `references/refactor-templates.md` 참조.
...
안내 템플릿은 `references/refactor-templates.md` 참조.

### 8. 코드 재생성 (선택)

AskUserQuestion으로 `/compile --all --conflict overwrite` 실행 여부 확인.
플로우 상세는 `references/refactor-templates.md` 참조.
