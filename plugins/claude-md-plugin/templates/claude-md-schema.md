...
### Code Style
- 함수 설계: early return 패턴, guard clause 우선
- 불변성: const 우선, 재할당 지양
- 비동기: async/await 사용, callback 지양
- null 처리: nullish coalescing(??) 사용, optional chaining 활용
- 조건문: 단순 분기는 ternary, 3개 이상은 object map / switch
- 에러 처리: custom error class 사용, 에러 메시지에 context 포함
...
> **참고**: 들여쓰기, 따옴표, 세미콜론, 줄 길이 등 포맷팅 규칙은 lint/formatter가 자동으로 처리하므로 여기에 작성하지 않습니다. Code Style에는 lint로 강제할 수 없는 코드 작성 패턴과 설계 규칙만 기술합니다.
...
| Code Style | Yes | 코드 작성 패턴, 설계 규칙 (lint로 강제할 수 없는 규칙) |
