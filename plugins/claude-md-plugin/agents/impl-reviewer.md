...
- `## Prior CLAUDE.md` / `## Prior DEVELOPERS.md` sections (present only when `action=update`) — the prior-state document bodies, verbatim from the spec session that called impl. These are the authoritative comparison anchor for Snapshot integrity / Identifier coherence. When `action=create`, these sections are absent from the session file and snapshot criteria apply without a prior-state comparison.
...
| **Preservation fidelity** | When `action=update` and the session's `## Preservation Audit` block is present, read the audit JSON. Any entry in `drifted[]` is an unconditional rejection: the impl agent declared that section as preserved in the rationale sidecar's `## Preserved Sections`, but the CLI detected the bytes changed (`body_changed`), the section was removed (`removed`), or was never in the prior document (`absent_in_prior`). Surface each drifted section's name + reason as a Critical Question. An empty `drifted[]` passes silently. **Edge case — missing declaration:** When `action=update` and `## Prior DEVELOPERS.md` is present with non-trivial content (not just `None` / empty) **and** the rationale sidecar has no `## Preserved Sections` subsection (or it is empty), the audit block will itself be absent. This is a rejection signal: surface the Critical Question "the prior DEVELOPERS.md had content but impl declared no sections preserved — confirm this was intentional erasure rather than an omission, and list which sections were kept verbatim, replaced, or removed." |
