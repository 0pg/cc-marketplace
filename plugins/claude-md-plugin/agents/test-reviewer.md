...
2. 추출 대상: `behaviors`, `exports`, `contracts`

### Phase 2: 테스트 파일 분석

1. 각 테스트 파일 `Read(test_file)` → 테스트 코드 로드
2. 테스트 케이스 추출: 테스트 이름/설명, 테스트 대상, assertion 목록
...
**약한 assertion (감점):** `toBeTruthy()`, `toBeDefined()`, `not.toBeNull()` 만 사용
**강한 assertion (만점):** `toEqual(expectedValue)`, 특정 속성 값 검증, 에러 타입 검증
...
error/edge behavior가 없으면 100% (N/A).
...
소수점 이하 반올림. **100% 필수**: 모든 5개 check가 만점이어야만 approve.
...
결과를 `.claude/tmp/{session-id}-test-review-{target}.json`에 저장합니다.
