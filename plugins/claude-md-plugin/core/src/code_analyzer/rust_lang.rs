...
mod tests {
    ...
    fn analyze(content: &str) -> PartialAnalysis {
        ...
        analyzer.analyze_file(Path::new("lib.rs"), content).unwrap()
    }

    #[test]
    fn test_pub_function() {
        let result = analyze("pub fn validate(token: &str) -> Result<Claims, Error> {\n}\n");

        assert_eq!(result.functions.len(), 1);
        assert_eq!(result.functions[0].name, "validate");
        assert!(result.functions[0].signature.contains("fn validate(token: &str) -> Result<Claims, Error>"));
    }

    #[test]
    fn test_pub_function_no_return() {
        let result = analyze("pub fn setup(config: &Config) {\n}\n");

        assert_eq!(result.functions.len(), 1);
        assert_eq!(result.functions[0].signature, "fn setup(config: &Config)");
    }

    #[test]
    fn test_private_function_excluded() {
        let content = "pub fn public_fn() {}\nfn private_fn() {}\n";
        let result = analyze(content);

        assert_eq!(result.functions.len(), 1);
        assert_eq!(result.functions[0].name, "public_fn");
    }

    #[test]
    fn test_generic_function() {
        let result = analyze("pub fn parse<T: Deserialize>(data: &str) -> T {\n}\n");

        assert_eq!(result.functions.len(), 1);
        assert_eq!(result.functions[0].name, "parse");
    }

    #[test]
    fn test_pub_struct() {
        let result = analyze("pub struct TokenService {\n    secret: String,\n}\n");
    ...
    #[test]
    fn test_pub_enum() {
        let result = analyze("pub enum Status {\n    Active,\n    Inactive,\n}\n");
...
"#;
        ...
        let errors: Vec<_> = result.behaviors.iter()
            ...
            .collect();
        assert!(errors.iter().any(|b| b.output == "AuthError::Expired" && b.input == "Expired token"));
        assert!(errors.iter().any(|b| b.output == "AuthError::InvalidFormat" && b.input == "Invalid token"));
    }

    #[test]
    fn test_use_dependencies() {
        let content = r#"
...
"#;
        ...
        assert!(result.external_deps.contains(&"serde".to_string()));
        assert!(result.external_deps.contains(&"tokio".to_string()));
        // std, crate, super should be excluded
        assert!(!result.external_deps.contains(&"std".to_string()));
        assert!(!result.external_deps.contains(&"crate".to_string()));
        assert!(!result.external_deps.contains(&"super".to_string()));
    }

    #[test]
    fn test_derive_macro_dependency() {
        let content = r#"
...
"#;
        ...
        assert!(result.external_deps.contains(&"thiserror".to_string()));
    }

    #[test]
    fn test_contract_from_doc_comments() {
        let content = r#"
...
/// # Errors
///
/// - `AuthError::Expired` if token is expired
/// - `AuthError::Invalid` if token is malformed
...
"#;
        ...
        assert_eq!(contract.function_name, "validate");
        assert!(!contract.contract.preconditions.is_empty());
        assert!(!contract.contract.postconditions.is_empty());
        // Errors section must have backtick-wrapped error types
        assert!(!contract.contract.throws.is_empty(), "expected throws from # Errors section");
    }

    #[test]
    fn test_protocol_from_state_enum() {
        let content = r#"
...
"#;
        ...
        assert!(protocol.states.contains(&"Disconnected".to_string()));
    }

    #[test]
    fn test_success_behavior_for_validate() {
        let content = "pub fn validate_token(token: &str) -> Result<Claims, Error> {}\n";
        let result = analyze(content);

        let success = result.behaviors.iter().find(|b| b.category == BehaviorCategory::Success);
        assert!(success.is_some());
        assert_eq!(success.unwrap().output, "Ok(Claims)");
    }
