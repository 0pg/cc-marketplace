...
mod tests {
    ...
    fn analyze(content: &str) -> PartialAnalysis {
        ...
        analyzer.analyze_file(Path::new("TokenService.java"), content).unwrap()
    }

    #[test]
    fn test_public_method() {
        let result = analyze("public Claims validateToken(String token) {}\n");

        assert_eq!(result.functions.len(), 1);
        assert_eq!(result.functions[0].name, "validateToken");
        assert_eq!(result.functions[0].signature, "Claims validateToken(String token)");
    }

    #[test]
    fn test_static_method() {
        let result = analyze("public static Claims parse(String data) {}\n");
    ...
    fn test_constructor_excluded() {
        let result = analyze("public TokenService(String secret) {}\npublic Claims validate(String token) {}\n");

        // Constructor name matches class name (TokenService.java -> TokenService)
        assert_eq!(result.functions.len(), 1);
        assert_eq!(result.functions[0].name, "validate");
    }

    #[test]
    fn test_getter_setter_excluded() {
        let content = r#"
...
"#;
    ...
    fn test_public_class() {
        let content = "public class AuthService {\n}\n";
        let result = analyze(content);
    ...
    fn test_class_extends() {
        let content = "public class AuthService extends BaseService {\n}\n";
        let result = analyze(content);

        assert_eq!(result.classes.len(), 1);
        assert_eq!(result.classes[0].signature.as_deref(), Some("class AuthService extends BaseService"));
    }

    #[test]
    fn test_exception_class() {
        let content = "public class InvalidTokenException extends RuntimeException {\n}\n";
        let result = analyze(content);

        assert_eq!(result.classes.len(), 1);
        assert!(result.classes[0].signature.as_deref().unwrap().contains("Exception"));
    }

    #[test]
    fn test_public_interface() {
        let result = analyze("public interface Validator {\n    boolean validate(String input);\n}\n");
    ...
    #[test]
    fn test_public_enum() {
        let result = analyze("public enum Status {\n    ACTIVE, INACTIVE\n}\n");
...
"#;
        ...
        // java.* and javax.* should be excluded
        assert!(!result.external_deps.iter().any(|d| d.starts_with("java")));
        assert!(!result.external_deps.iter().any(|d| d.starts_with("javax")));
        assert!(result.external_deps.contains(&"io.jsonwebtoken".to_string()));
        assert!(result.external_deps.contains(&"com.google".to_string()));
    }

    #[test]
    fn test_contract_from_javadoc() {
        let content = r#"
/**
...
"#;
        ...
        assert!(contract.contract.throws.contains(&"ExpiredTokenException".to_string()));
    }

    #[test]
    fn test_behavior_from_throws_clause() {
        let content = "public Claims validate(String token) throws InvalidTokenException, ExpiredTokenException {}\n";
        let result = analyze(content);
...
"#;
    ...
    fn test_protocol_from_sealed_class() {
        let content = "public sealed class State permits Idle, Loading, Loaded, Error {\n}\n";
        let result = analyze(content);
    ...
    fn test_success_behavior_for_validate() {
        let content = "public Claims validate(String token) {}\n";
        let result = analyze(content);

        let success = result.behaviors.iter().find(|b| b.category == BehaviorCategory::Success);
        assert!(success.is_some());
        assert_eq!(success.unwrap().output, "TokenClaims object");
    }
