Feature: CLAUDE.md Parser
  ...
  # =============================================================================
  # v2 Exports Parsing (#### heading format)
  # =============================================================================

  Scenario: Parse v2 exports with heading format
    Given a CLAUDE.md file with content:
      """
      <!-- schema: 2.0 -->
      # auth-module
      ...
      - **Type**: type
      - **Signature**: `Claims { userId: string, role: Role, exp: number }`
    ...
    Then the spec should have exported functions:
      | name          | signature                                     |
      | validateToken | validateToken(token: string): Promise<Claims> |
    And the spec should have exported types:
      | name   | definition                                    |
      | Claims | Claims { userId: string, role: Role, exp: number } |

  Scenario: Parse v2 mixed export types (function + class + type + enum + variable)
    Given a CLAUDE.md file with content:
      """
      <!-- schema: 2.0 -->
      # mixed-module
    ...
    Then the spec should have exported functions:
      | name        | signature                        |
      | processData | processData(input: Data): Result |
    And the spec should have exported classes:
      | name          | constructor_signature       |
      | DataProcessor | DataProcessor(config: Config) |
    And the spec should have exported types:
      | name       | definition                                       |
      | DataConfig | DataConfig { timeout: number, retries: number } |

  Scenario: Parse v2 exports with anchor information
    Given a CLAUDE.md file with content:
      """
      <!-- schema: 2.0 -->
      # anchor-module
    ...
    Then the spec should have exported functions:
      | name        | signature                       |
      | formatError | formatError(err: Error): string |
