...
fn main() {
    ...
    let schema_path = "../references/shared/schema-rules.yaml";
    ...
    let yaml_content = fs::read_to_string(schema_path).unwrap_or_else(|e| {
        panic!(
            "Failed to read schema-rules.yaml at '{}': {}. \
             Make sure the file exists at plugins/claude-md-plugin/references/shared/schema-rules.yaml",
            schema_path, e
        )
    });
    ...
    // Extract DEVELOPERS.md conditionally required sections
    let mut developers_conditional_sections: Vec<(&str, &str)> = rules
        .developers_sections
        .values()
        .filter(|s| s.required && !s.condition.is_empty())
        .map(|s| (s.name.as_str(), s.condition.as_str()))
        .collect();
...
// DO NOT EDIT MANUALLY - Edit references/shared/schema-rules.yaml instead
...
/// DEVELOPERS.md conditionally required sections (name, condition)
#[allow(dead_code)]
